function [J, Trec, Tsim_p, f_h, f_h1] = Calcul_FFT_fonc_air_2(p,t_ini,t_fin,dt_sim,dt_T,Tsim,Teau,Tair,OK_des)
% Fonction-coût de détermination des paramètres
% alfa et Kair en maintenant la constante de temps eta fixe
global etaest0
alfa = p(1);
Kair = p(2);
eta = etaest0;
...
% Calcul des tendances qui vont être enlevées pour identifierTeau_tend = 0*u_cs*(u_cs\Teau_p);
...
J = Jadeq+1e6*min(alfa,0)^2+1e6*min(Kair-0.5,0)^2+1e6*min(eta,0)^2+... 1e6*min(p(3),0)^2+...
